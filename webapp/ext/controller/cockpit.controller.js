sap.ui.define(
  ...
  function (ControllerExtension, MessageToast) {
    ...
    return ControllerExtension.extend("cockpit.ext.controller.cockpit", {
      //private properties
      _plant: null,
      _site: null,
      ...
      onCancelCounting: async function (oBindingContext, aSelectedContexts) {
        ...
        sap.m.MessageBox.confirm("Would you like to perform the action", {
          ...
          onClose: this.onActionConfirmCancelCounting.bind(
            this,
            aSelectedContexts,
          ),
        });
        ...
        try {
          ...
          await Promise.all(
            aContexts.map(async (oContext) => {
              ...
              const oAction = oModel.bindContext(
                `${sPath}/com.sap.gateway.srvd.zr_wm101_cockpit.v0001.cancel_counting(...)`,
              );
        ...
        try {
          ...
          await Promise.all(
            aContexts.map(async (oContext) => {
              const sPath = oContext.getPath();
              const oAction = oModel.bindContext(
                `${sPath}/com.sap.gateway.srvd.zr_wm101_cockpit.v0001.complete_counting(...)`,
              );
              await oAction.execute();
            }),
          );
